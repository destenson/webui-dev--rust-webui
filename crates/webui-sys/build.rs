    ...
    P: AsRef<Path>,
{
    bindgen::Builder::default()
        .headers([
            ...
            //         Some(s)
            //     })
            //     .ok_or_else(|| Error::Unknown("cannot create path to webui.hpp".to_string()))?
            ]
        )

        // include webui symbols
        .allowlist_function("webui.*")
        .allowlist_item("webui.*")
        .allowlist_type("webui.*")
        .allowlist_var("webui.*")

        // block all other symbols
        .blocklist_function("[a-vx-zA-VXZ0-9_]+")
        .blocklist_item("[a-vx-zA-VXZ0-9_]+")
        .blocklist_type("[a-vx-zA-VXZ0-9_]+")
        .blocklist_var("[a-vx-zA-VXZ0-9_]+")

        // generate the bindings & write the file
        .generate()?
...
fn main() {
    ...
    if cfg!(not(feature = "runtime")) {
        ...
        println!("cargo:rustc-link-lib=webui-2");
        println!("cargo:warning=Not using runtime feature");
    }
...
fn download_or_find_webui_root(out_dir: &str) -> String {
    match get_env_var("WEBUI_ROOT") {
        ...
        Err(_) => {
            ...
            let output_path = format!("{}{}-{}", &output_dir, version,
